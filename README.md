...
### 현재 상태
책·메모·태그 CRUD, 지식 정원, 스트릭, Vercel 배포까지 동작 중. 아래는 **앱 기능** 기준 보완점이다.

### 기능 단위

| 기능 | 보완점 |
|---|---|
| 서재 | 목록에서 상태만 보이고, 변경은 상세에서만 가능 → 의도된 UX이나 상태 일괄 변경은 없음 |
| 메모 | 책 필수·태그 연동은 됨. 메모 목록 검색/필터 없음 |
| 태그 | 관리·메모 연결은 됨. 태그별 메모 모아보기 없음 |
| 지식 정원 | 공통 태그 없으면 선이 안 생김. 노드 탭 시 메모 상세 이동 없음 |
| 스트릭 | UI는 `book_notes.created_at` 집계. DB `streaks` 테이블과 미연동 |
| 성능 | Force Graph가 초기 번들에 포함 → 정원 탭 lazy load 가능 |

### 필수 / 선택

| 구분 | 항목 | 파일 | 확인 |
|---|---|---|---|
| **필수** | 스트릭 집계를 `streaks`와 맞추거나, UI 기준을 코드 주석/스키마와 일치시키기 | `useReadingStreak.ts`, `build-grass.ts` | 오늘 메모 작성 → 잔디·연속일 증가 |
| **필수** | 정원: 공통 태그 없을 때 안내가 충분한지 점검 | `KnowledgeGardenGraph.tsx` | 태그 없는 메모만 있을 때 Empty/안내 문구 |
| **선택** | 메모·서재 검색/필터 | `MemoList`, `BookList` | 키워드로 목록 축소 |
| **선택** | 정원 노드 클릭 → 메모 수정 화면 | `KnowledgeGardenGraph.tsx` | 노드 탭 시 `/memos/:id/edit` |
| **선택** | Graph 라우트 lazy import | `router/index.tsx` | 첫 로딩 후 정원 탭만 추가 로드 |
| **선택** | 태그별 메모 목록 | `TagsPage` / tags feature | 태그 선택 시 관련 메모 표시 |

### README에 적을 기능 메모 (짧게)
- 스트릭 = 메모 **생성일** 기준  
- 정원 = **같은 태그**를 공유하는 메모끼리 연결  
